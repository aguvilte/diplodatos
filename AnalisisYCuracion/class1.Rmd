...
```{r}
m=10; n=10;
v1<-replicate(m, rnorm(n))
system.time(
  for (i in 1:m) {
    for (j in 1:n) {
      v1<-v1 + 10*sin(0.75*pi)
    }
  }
)
```
...
filteredFlights <- (arr_delay>=2)
paste("Cantidad de vuelos que arribaron con un retraso de más de 2 horas:", table(filteredFlights)["TRUE"])

filteredFlights <- (dest %in% c("IAH", "HOU"))
paste("Cantidad de vuelos hacia Houston:", table(filteredFlights)["TRUE"])

filteredFlights <- (carrier %in% c("UA", "AA", "DL"))
paste("Cantidad de vuelos operados por United, American o Delta:", table(filteredFlights)["TRUE"])

filteredFlights <- (month>=7 & month<=9 )
paste("Cantidad de vuelos que salieron en Julio, Agosto y Septiembre:", table(filteredFlights)["TRUE"])

filteredFlights <- (arr_delay>=2 & sched_dep_time == dep_time )
paste("Cantidad de vuelos que arribaron mas de dos horas tarde, pero salieron bien:", table(filteredFlights)["TRUE"])

filteredFlights <- (dep_time>=0 & dep_time<=600 )
paste("Cantidad de vuelos que salieron entre medianoche y las 6 am:", table(filteredFlights)["TRUE"])
```
